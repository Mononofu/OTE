#ifndef FEED_DATA_TYPES_H
#define FEED_DATA_TYPES_H
...
/** \file 
 * Available InformationManager feeds.
 * - app_event: exitting, minimizing, etc.
 * - thread_event: same for the different threads
 * - input_mouse: all mouse input
 ...
 * - gui_event: everything that happens in the gui
 **/
...
/** Data type for mouse events.
 * Contains current absolute and relative position as well as the last pressed button and the current state
 **/
...
struct InputMouseEvent
{
	...
	int mouseDeltaY;
	input_action action;
	input_mouse_type type;
};

// Datatype for feed 'input_keyboard'
...
namespace boost {
	namespace serialization {
		...
		template<class Archive>
				void serialize(Archive & ar, OgreNewt::Node & node, const unsigned int version)
		{
			ar & node.pos.x;
			ar & node.pos.y;
			ar & node.pos.z;
...
struct WorldGraph
{
	std::vector< OgreNewt::Node* > nodes;
	void addNode(OgreNewt::Node* node) { nodes.push_back(node); }
	WorldGraph() {}
	WorldGraph(const WorldGraph& source)
	{
		if(!source.nodes.empty())
		{
			foreach(OgreNewt::Node* node, source.nodes)
			{
				this->nodes.push_back(new OgreNewt::Node(node));
			}
		}
	...
	WorldGraph& operator=(const WorldGraph& source)
	{
		...
		if(!nodes.empty())
		{
			foreach(OgreNewt::Node* node, nodes)
			{
				delete node;
			}
			nodes.clear();
